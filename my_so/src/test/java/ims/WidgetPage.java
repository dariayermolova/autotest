...
public class WidgetPage extends test {
    ...
    @Story("Login")
        ...
        switchTo().window(1);
       $(By.className("icon-facebook")).waitUntil(visible, 20000).shouldHave(Condition.text("F"));
    ...
    @Story("Login")
      public void t5_LoginViaGoogle() {
        ...
        $(By.id("identifierId")).waitUntil(visible, 20000).setValue(emailG);
    ...
    @Feature("IMS")
    @Story("Login")
    public void t6_LoginViaFacebook() {
        open(urlWidgets);
        Selenide.clearBrowserCookies();
        refresh();
        switchTo().frame($(By.xpath("//div[@class='so_comments']//iframe")));
        ...
        screenshot("before");
        $(By.id("email")).waitUntil(visible,20000).setValue(emailFD);
