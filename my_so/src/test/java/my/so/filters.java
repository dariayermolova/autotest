...
public class filters extends suite {
    ...
    public void t3_CheckBlacklist() {
        $(By.xpath("//button[@href='#modalAddFilter']")).scrollTo().click();
        $(By.id("modalAddFilter")).shouldBe(Condition.visible);
        $(By.id("filter_value")).setValue("checkmoderator@mailinator.com").pressEnter();
        $(By.className("autocomplete-suggestion")).waitUntil(Condition.visible, 30000).click();
        $(By.id("filter_description")).setValue("spam");
        $(By.id("save_filter")).click();
        $(By.className("filter_email")).shouldHave(Condition.text("checkmoderator@mailinator.com"));
        $(By.xpath("//div[@data-filter_value='checkmoderator@mailinator.com']")).click();
